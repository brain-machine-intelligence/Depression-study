...
myArbitrator.Time_Step=param_fixed(3); % the smaller, the slower
        ...
        % SARSA
        param_sarsa.alpha=param_in(pop_id,7); % learning rate (0.1~0.2)
        % FWD
        param_fwd.alpha=param_in(pop_id,8);
        % Softmax parameter for all models
        myArbitrator.tau_softmax_goal=param_in(pop_id,5); % use the same value as sarsa/fwd
        myArbitrator.tau_softmax_flex=param_in(pop_id,6); % use the same value as sarsa/fwd
        param_sarsa.tau=0.2;
        param_fwd.tau=0.2;        
...
%                 disp(sprintf('- generating regressor for SBJ%02d/%02d...',ll,tot_num_sbj));
                ...
                data_in{1,ll}.regressor{1,14}.name='Computational choice';     data_in{1,ll}.regressor{1,14}.value=[]; %
                ...
                data_in{1,ll}.regressor{1,28}.name='Tau';     data_in{1,ll}.regressor{1,28}.value=[]; % PMF after the update (the same as 'weightM2' but tested within the SPM design matrix)
                data_in{1,ll}.regressor{1,28}.Tag={'row1: session';'row2: block';'row3: trial in block';'row4: trial_s in trial';'row5: goal';'row6: active_model';'row7: tau'};                
                data_in{1,ll}.regressor{1,29}.name='absRPEestimator';     data_in{1,ll}.regressor{1,29}.value=[];
                data_in{1,ll}.regressor{1,29}.Tag={'row1: session';'row2: block';'row3: trial in block';'row4: trial_s in trial';'row5: goal';'row6: active_model';'row7: absRPEestimator'}; 
                que_window=1;
...
%             param_fwd.alpha=param_fwd.alpha/0.5;
            ...
            loglik_sess = zeros(1,num_max_session);
...
%                                 vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator_top.Q(1,action_chosen)]; % session; block; trial; trial_s; goal; active_model; choice prob of m1                                
                                ...
                                if action_chosen==1
                                    action_unchosen = 2;
                                elseif action_chosen==2
                                    action_unchosen = 1;
                                end
                                if myArbitrator_top.Q(1,action_chosen)>=myArbitrator_top.Q(1,action_unchosen)
                                    ttt=1;
                                else
                                    ttt=-1;
                                end
                                myArbitrator.tau_softmax = (1-myArbitrator_top.m1_prob)*myArbitrator.tau_softmax_flex + myArbitrator_top.m1_prob*myArbitrator.tau_softmax_goal;
                                var_exp=exp(myArbitrator.tau_softmax*myArbitrator_top.Q(1,:)); % (N_actionx1)
                                Prob_action=var_exp/sum(var_exp); % (N_actionx1)
                                vec_reg=[an_s; an_i; an_ib; an_j; an_g; ttt; Prob_action(action_chosen)]; % session; block; trial; trial_s; goal; active_model; Q-value for left and right                                           
                                ...
                                an_j=1;
                                myArbitrator.tau_softmax = (1-myArbitrator_top.m1_prob)*myArbitrator.tau_softmax_flex + myArbitrator_top.m1_prob*myArbitrator.tau_softmax_goal;
                                vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator.tau_softmax]; % session; block; trial; trial_s; goal; active_model; updated weightM1 (1x1);
                                data_in{1,ll}.regressor{1,28}.value=[data_in{1,ll}.regressor{1,28}.value vec_reg];
                                % regressor29- m2_absPEestimate
                                an_j=1;
                                vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; 0]; % session; block; trial; trial_s; goal; active_model; updated weightM1 (1x1);
                        ...
                        myArbitrator_top.tau_softmax = (1-myArbitrator_top.m1_prob)*myArbitrator.tau_softmax_flex + myArbitrator_top.m1_prob*myArbitrator.tau_softmax_goal;
                        ...
                        loglik_sess(ind_sess) = loglik_sess(ind_sess)-eval_num/(mode.total_simul);
...
%                                 vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator_top.Q(state_data,action_chosen)]; % session; block; trial; trial_s; goal; active_model; x                                
                            ...
                            if an_j==2
                                ...
                                if myArbitrator_top.Q(state_data,action_chosen)>=myArbitrator_top.Q(state_data,action_unchosen)
                                    ttt=1;
                                else
                                    ttt=-1;
                                end 
                            ...
                            myArbitrator.tau_softmax = (1-myArbitrator_top.m1_prob)*myArbitrator.tau_softmax_flex + myArbitrator_top.m1_prob*myArbitrator.tau_softmax_goal;
                            an_j=myArbitrator_top.index+1; % trial_s in trial (2 or 3)
                            vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator.tau_softmax];
%                             if(an_j==2)
%                                 vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator.tau_softmax]; % session; block; trial; trial_s; goal; active_model; updated weightM1 (1x1);
%                             else % no PMF computation because there is no need for value computation
%                                 vec_reg=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; 0]; % session; block; trial; trial_s; goal; active_model; x
%                             end
                            vec_reg1=[an_s; an_i; an_ib; an_j; an_g; myArbitrator_top.ind_active_model; myArbitrator_top.m2_absPEestimate];
                            data_in{1,ll}.regressor{1,28}.value=[data_in{1,ll}.regressor{1,28}.value vec_reg];     
                            data_in{1,ll}.regressor{1,29}.value=[data_in{1,ll}.regressor{1,29}.value vec_reg1];   
                        end
...
data_in{1,ll}.neglog = Sum_NegLogLik_each_simul_eachMode;
data_in{1,ll}.session = loglik_sess;
%% function returns
if(mode.out==0)
    data_out=OBS;
end
